export const utilsTs = `export function getCircleById(circles: Circles, id: string | undefined) {
...
export function getCircleUnderPointer(
  ...
  return circles.find((circle: Circle) => {
    ...
    const d = Math.sqrt(
      Math.pow(circle.position.x - position.x, 2) +
        Math.pow(circle.position.y - position.y, 2)
    );
