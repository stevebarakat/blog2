...
In this excercise, we'll be re-create the same example from the previous lesson using `createActorContext` to work with context. `createActorContext` is a function that creates a context object that can be used to store data in a state machine. Here's an example of a state machine that stores the current volume level in the context using `createActorContext`:
