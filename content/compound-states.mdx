---
title: Working with Compound States
abstract: In this lesson, we'll begin working with statecharts by using compound (or parent/child) states.
position: 3
---

<div className="container">
In this lesson, we'll  compound (or parent/child) states. Compound states are states that contain other states. They are useful for organizing and structuring complex state machines. Compound states can have nested states, which can have their own nested states, and so on. This allows you to create a hierarchical structure of states that can represent complex behavior.

Let's take a look at an example of a simple state machine with compound states (technically a statechart). This statechart represents a media player that can be in one of three states: "Opened", "Closed", and "Closing". The "Closed" state is a parent state. It has three child states: "Stopped", "Playing", and "Paused". The "Closing" state transitions to the "Closed.Playing" state after 1500ms. Here's the state machine defined in Stately:

</div>

<Stately
  url="https://stately.ai/registry/editor/embed/6ad630e7-c4f1-40e5-8068-4c0a0398a8e9?machineId=f5af41be-b005-4bb3-834c-592861cfde06"
  height="650px"
/>

<div className="container">
In XState, a state machine is defined using a JSON-like object. Here's the same state machine defined in XState:

```ts
import { setup } from "xstate";

export const machine = setup({
  types: {
    events: {} as
      | { type: "CLOSE" }
      | { type: "PLAY" }
      | { type: "PAUSE" }
      | { type: "STOP" }
      | { type: "OPEN" },
  },
}).createMachine({
  context: {},
  id: "Parent states",
  initial: "Opened",
  states: {
    Opened: {
      on: {
        CLOSE: {
          target: "Closed",
        },
        PLAY: {
          target: "Closing",
        },
      },
    },
    Closed: {
      initial: "Stopped",
      on: {
        OPEN: {
          target: "Opened",
        },
      },
      states: {
        Stopped: {
          on: {
            PLAY: {
              target: "Playing",
            },
          },
        },
        Playing: {
          on: {
            PAUSE: {
              target: "Paused",
            },
            STOP: {
              target: "Stopped",
            },
          },
        },
        Paused: {
          on: {
            PLAY: {
              target: "Playing",
            },
            STOP: {
              target: "Stopped",
            },
          },
        },
      },
    },
    Closing: {
      after: {
        "1500": {
          target: "#Parent states.Closed.Playing",
        },
      },
    },
  },
});
```

To use this machine in a React component, we can use the `useMachine` hook from `@xstate/react`:

```tsx
import { machine } from "./machine";
import { useMachine } from "@xstate/react";

function App() {
  const [state, send] = useMachine(machine);
  const isPlaying = state.matches({ Closed: "Playing" });
  const isOpened = state.matches("Opened");
  return (
    <div>
      <h1>{JSON.stringify(state.value)}</h1>
      <button
        onClick={() =>
          send({
            type: isOpened ? "CLOSE" : "OPEN",
          })
        }
      >
        {isOpened ? "close" : "open"}
      </button>
      <button
        onClick={() =>
          send({
            type: isPlaying ? "PAUSE" : "PLAY",
          })
        }
      >
        {isPlaying ? "pause" : "play"}
      </button>
      <button
        disabled={isOpened || !isPlaying}
        onClick={() => {
          send({ type: "STOP" });
        }}
      >
        stop
      </button>
    </div>
  );
}

export default App;
```

As you can see we are using the `useMachine` hook to get the current `state` and the `send` function to dispatch events to the machine. When the button is clicked, the "next" event is sent to the machine, which transitions to the next state. The current state is displayed in a paragraph element. Below is the state machine in action:

</div>

<CodeEditor
  files={{
    starter: {
      "/App.js": {
        code: `import { machine } from "./machine";
import { useMachine } from "@xstate/react";

function App() {
const [state, send] = useMachine(machine);
return (

<>
  <p>{state.value}</p>
  <button
    onClick={() =>
      send({
        type: "next",
      })
    }
  >
    next
  </button>
</>
); }

export default App;`,
      },
      "/machine.ts": {
        code: `import { setup } from "xstate";

export const machine = setup({
  types: {
    events: {} as { type: "next" },
  },
}).createMachine({
  initial: "First State",
  states: {
    "First State": {
      on: {
        next: {
          target: "Second State",
        },
      },
    },
    "Second State": {
      on: {
        next: {
          target: "First State",
        },
      },
    },
  },
});`,
      },
    },
  }}
/>

<div className="container">
Play around with the code above to see how the state machine transitions between states when the button is clicked. Here are some excercises to try:

<ul>
  <li>Change the name of the event to "toggle" and see how the machine behaves
  (make sure you do that in App.js and machine.ts).</li>

  <li>Change the name of the states to "Active" and "Inactive" and see how the machine behaves.</li>
</ul>
</div>


## Exercise
<div className="exercise">

<div className="requirements">
For this exercise create a simple state machine that:

- Toggles between two states: "On" and "Off".
- The machine should have a single event called "switch" that toggles between the two states.
- The initial state should be "Off".
- The button text should display the current state of the machine.
</div>


<CodeEditor files={
  { starter: {
  "/App.js": {
    code: `function App() {
  return (
    <div>
      <h1>Current state goes here</h1>
      <button>Weigh</button>
      <button>Grind</button>
      <button>Brew</button>
    </div>
  );
}

export default App;
`,
  },
  "/machine.ts": {
    active: true,
    code: `import { setup } from "xstate";

export const machine = setup({
  types: {
    events: {} as {},
  },
}).createMachine({
  initial: "",
  states: {},
});
`,
},
}, solution : {
  "/App.js": {
    code: `import { machine } from "./machine";
import { useMachine } from "@xstate/react";

function App() {
  const [state, send] = useMachine(machine);
  return (
    <div>
      <h1>{JSON.stringify(state.value)}</h1>
      <button onClick={() => send({ type: "weigh" })}>Weigh</button>
      <button onClick={() => send({ type: "grind" })}>Grind</button>
      <button onClick={() => send({ type: "brew" })}>Brew</button>
    </div>
  );
}

export default App;
`,
  },
  "/machine.ts": {
    active: true,
    code: `import { setup } from "xstate";

export const machine = setup({
  types: {
    context: {} as {},
    events: {} as { type: "weigh" } | { type: "grind" } | { type: "brew" },
  },
}).createMachine({
  context: {},
  id: "Coffee Machine",
  initial: "preparation",
  states: {
    preparation: {
      initial: "idle",
      states: {
        idle: {
          on: {
            weigh: {
              target: "weighed",
            },
          },
        },
        weighed: {
          on: {
            grind: {
              target: "ground",
            },
          },
        },
        ground: {
          on: {
            brew: {
              target: "#Coffee Machine.brewing",
            },
          },
        },
      },
    },
    brewing: {
      after: {
        "3000": {
          target: "ready",
        },
      },
    },
    ready: {
      type: "final",
    },
  },
});
`,
},
}}} />

</div>

